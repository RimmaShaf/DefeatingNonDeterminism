<script lang="ts">
	...
	// data/poem-variance-groq/latest.json for later replay.
	async function runLive() {
		const n = Math.max(1, Math.min(100, runCount));
		await fetchInto('live', () =>
			fetch(`${base}/api/poem-variance-groq/live`, {
				method: 'POST',
	...
	async function showSaved() {
		await fetchInto('saved', () => fetch(`${base}/api/poem-variance-groq`));
	}
</script>

<div class="pvd" data-module="poem-variance-groq">
	<h4 class="pvd__title">
		One prompt, 100× to the cloud: temperature&nbsp;=&nbsp;0, fixed seed, Groq API
	</h4>

	<p class="pvd__setup">
		The same prompt goes to <code>openai/gpt-oss-120b</code> on Groq one hundred times in a row
		with <code>temperature: 0</code> and a fixed <code>seed</code> — as deterministic as the API
		allows you to ask for. Below are the real responses: each line of the quatrain is colored by
		which variant of the text this particular run got.
	</p>
	...
	{#if response && successfulRuns.length > 0}
		<div class="pvd__source" class:pvd__source--live={source === 'live'}>
			{#if source === 'live'}
				● live run — JSON saved to <code>data/poem-variance-groq/latest.json</code>
			{:else}
	...
	<p class="pvd__takeaway">
		<!-- Even with a fixed seed, Groq only promises "best effort" determinism: dynamic batching,
		MoE routing, and distributed inference still leak variation into the output — which is why
		temperature: 0 + seed doesn't guarantee the same text twice. -->
	</p>
</div>
